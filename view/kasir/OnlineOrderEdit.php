        ...
        <div class="card" style="max-width: 600px;">
            <form method="POST" action="">
                <div class="form-group">
                    ...
                    <input type="text" value="<?= $data['bookingData']['kode_booking']; ?>" class="form-control" readonly style="background-color: #e2e8f0;">
                </div>

                <div class="form-group">
                    <label>Nama Tamu</label>
                    <input type="text" value="<?= $data['bookingData']['nama_lengkap']; ?>" class="form-control" readonly style="background-color: #e2e8f0;">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="id_kamar" class="form-control" required>
                        ...
                        <option value="<?= $data['bookingData']['id_kamar']; ?>" selected>
                            <?= $data['bookingData']['nomor_kamar']; ?> - <?= $data['bookingData']['nama_tipe']; ?> (Kamar Saat Ini)
                        </option>
                        <!-- Kamar tersedia -->
                        <?php foreach ($data['availableRooms'] as $room): ?>
                            <option value="<?= $room['id']; ?>">
                                <?= $room['nomor_kamar']; ?> - <?= $room['nama_tipe']; ?> (Rp <?= number_format($room['harga_per_malam'], 0, ',', '.'); ?>/malam)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tgl_check_in" value="<?= $data['bookingData']['tgl_check_in']; ?>" class="form-control" required>
                </div>

                <div class="form-group">
                    <label>Tanggal Check-Out <span style="color:red;">*</span></label>
                    <input type="date" name="tgl_check_out" value="<?= $data['bookingData']['tgl_check_out']; ?>" class="form-control" required>
                </div>

                <div class="form-group">
                    <label>Total Harga <span style="color:red;">*</span></label>
                    <input type="number" name="total_harga" value="<?= $data['bookingData']['total_harga']; ?>" class="form-control" required min="0" step="1000">
                </div>

                <div class="form-group">
                    <label>Status</label>
                    <input type="text" value="<?= ucfirst($data['bookingData']['status']); ?>" class="form-control" readonly style="background-color: #e2e8f0;">
                </div>
